...
/*   Updated: 2019/01/31 19:45:21 by nvienot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	win->hor = (IMG_HOR_SIZE - (win->s[win->pos_max]->x_pix - win->s[0]->x_pix)) / 2;
	win->ver = (IMG_VER_SIZE - (win->s[(win->pos_max - (win->x_max - 1))]->y_pix) - win->s[(win->x_max - 1)]->y_pix) / 2;
	return (1);
}
